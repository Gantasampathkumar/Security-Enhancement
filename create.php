...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        $stmt = $mysqli->prepare('INSERT INTO posts (title, content, author, user_id, created_at) VALUES (?, ?, ?, ?, NOW())');
        $stmt->bind_param('sssi', $title, $content, $author, $userId);

        if ($stmt->execute()) {
            $stmt->close();
            record_activity($mysqli, $userId, 'Create Post');
            header('Location: index.php?msg=' . urlencode('Post created successfully.') . '&type=success');
            exit;
        }

        $errors[] = 'Unable to create post. Please try again later.';
        $stmt->close();
    }
}
?>
...
<main class="container mt-4">
    <div class="page-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
        <div>
            <h1 class="display-6 mb-2">Create New Post</h1>
            <p class="text-muted mb-0">Use the secure form below to add a fresh blog post.</p>
        </div>
